...
#define RG_DEFINE_PASS_INPUTS(Type, ...) \
    friend bool operator==(const Type& a, const Type& b) { return ::org::AutoEqualMembers<Type, __VA_ARGS__>(a, b); } \
    friend ::org::Hash64 HashValue(const Type& value) { return ::org::AutoHashMembers<Type, __VA_ARGS__>(value); }

struct AnyPassInputs {
    const std::type_info* type = nullptr;

    using HashFn = org::Hash64(*)(const void*) noexcept;
    ...
    template<org::PassInputs T>
    void set(T value) {
        reset();
        ptr = new T(std::move(value));
        type = &typeid(T);

        hashFn = [](const void* p) noexcept -> org::Hash64 {
    ...
    org::Hash64 hash() const noexcept { return hashFn(ptr); }
...
public:
    ...
    template<org::PassInputs T>
    void SetInputs(T in) {
        org::Hash64 newKey = Mix(TypeHash<T>(), HashValue(in));
...
private:
    ...
    org::Hash64 compileKey_ = 0;
    bool compileDirty_ = true;

    template<class T> static org::Hash64 TypeHash() noexcept {
        // Use a stable per-run type hash. If you want cross-run stability, supply your own ID.
        return org::Hash64(std::type_index(typeid(T)).hash_code());
    }

    static org::Hash64 Mix(org::Hash64 a, org::Hash64 b) noexcept {
